...
(function() {
  var TRM, alert, badge, check_nws, debug, echo, glob, help, info, log, njs_fs, rainbow, rpr, try_escodegen, try_escodegen_1, try_esprima, try_esquery, try_splice, warn, whisper, π,
    __slice = [].slice;
  ...
  try_escodegen_1 = function() {
    var ESCODEGEN, ESPRIMA, NAME, escodegen_options, node;
  ...
  try_esquery = function() {
    var ESCODEGEN, ESPRIMA, ESQUERY, ESTRAVERSE, JSONSelect, literals, node, registry, _i, _len, _ref;
    ESCODEGEN = require('escodegen');
    ESPRIMA = require('esprima');
    ...
    ESTRAVERSE.traverse(node, {
      enter: function(node, parent) {
        ...
        if (parent != null) {
          return node['x-parent-id'] = parent['x-id'];
        }
      }
    });
    debug(registry[0]);
    _ref = ESQUERY.query(node, 'Literal');
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      node = _ref[_i];
      info(node);
    }
    return debug(ESQUERY.query(node, '[x-subtype="yay"]'));
  };
