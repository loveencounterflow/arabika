...
(function() {
  var $new, CHR, TRM, TYPES, XRE, alert, badge, debug, echo, help, info, log, rainbow, rpr, splice, warn, whisper, π,
    __slice = [].slice;
  ...
  this.$new.$phrase = function(G, $) {
    ...
    R = R.onMatch(function(match) {
      return $new.literal('phrase', match[0], match[0], match[0]);
    });
    return R;
  };
  ...
  this.$new.$chunks = function(G, $) {
    var R;
    R = π.alt(π.repeat((function(_this) {
      return function() {
        return G.$chunk;
      };
    })(this)));
    R = R.onMatch(function(match) {
      var element, idx, _i, _ref;
      for (idx = _i = _ref = match.length - 1; _i >= 0; idx = _i += -1) {
        element = match[idx];
        if ((TYPES.isa_list(element)) && element[0] === 'phrases') {
          whisper('©442', element);
          delete match[idx];
          splice(match, element.slice(1), idx);
        }
      }
      return match;
    });
  ...
  this.$TESTS = {
    ...
    'expression: parses simple bracketed phrase': function(test) {
      ...
      source = "(xxx)";
      return test.eq(G.$chunk.run(source), ["bracketed", "(", [["phrases", ["phrase", "xxx"]]], ")"]);
    },
    'expression: parses nested bracketed phrase': function(test) {
      ...
      source = "(A(B)C)";
      return test.eq(G.$chunk.run(source), ["bracketed", "(", [["phrases", ["phrase", "A"]], ["bracketed", "(", [["phrases", ["phrase", "B"]]], ")"], ["phrases", ["phrase", "C"]]], ")"]);
    },
    'expression: parses multiply nested bracketed phrase': function(test) {
      ...
      source = "(xxx(yyy(zzz))aaa)";
      return test.eq(G.$chunk.run(source), ["bracketed", "(", [["phrases", ["phrase", "xxx"]], ["bracketed", "(", [["phrases", ["phrase", "yyy"]], ["bracketed", "(", [["phrases", ["phrase", "zzz"]]], ")"]], ")"], ["phrases", ["phrase", "aaa"]]], ")"]);
    },
    'expression: parses multiply nested bracketed phrase with connectors': function(test) {
      ...
      source = "(xxx|www|333(yyy(zzz))aaa)";
      return test.eq(G.$chunk.run(source), ["bracketed", "(", [["phrases", ["phrase", "xxx"], ["phrase", "www"], ["phrase", "333"]], ["bracketed", "(", [["phrases", ["phrase", "yyy"]], ["bracketed", "(", [["phrases", ["phrase", "zzz"]]], ")"]], ")"], ["phrases", ["phrase", "aaa"]]], ")"]);
    },
    ...
    'expression (default G): parses one-liner program': function(test) {
      var G, source;
      G = this;
      source = "xxx";
      return debug(G.expression.run(source));
    },
    'expression (default G): parses program': function(test) {
      var G, node, source;
      G = this;
      source = "f = ->\n  for x in xs\n    while x > 0\n      x -= 1\n      log x\n      g x\n  log 'ok'\n  log 'over'";
      node = debug(G.expression.run(source));
      return debug(test.as_js(node));
    }
  };
