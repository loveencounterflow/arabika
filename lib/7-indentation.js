...
(function() {
  ...
  this.$new.$_raw_indented_material_line = function(G, $) {
    var R;
    R = ƒ.or(function() {
      return ƒ.seq(G.$_indentation, CHR.$nws, /.*/, ƒ.optional('\n'));
    });
    R = R.onMatch(function(match) {
  ...
  this.$new.$step = function(G, $) {
    var R;
    R = ƒ.seq((function() {
      return $.LINE.line;
    }), (function() {
      return ƒ.check(G.$indent);
    }));
  ...
  this.$TESTS = {
    ...
    '$indent: parses opener': function(test) {
      var G, source;
      G = this.$new({
        opener: '<',
        connector: '=',
      ...
      source = '<';
      return test.eq(G.$indent.run(source), '<');
    },
    '$step: parses line plus indent': function(test) {
      var G, source;
      G = this;
      source = "loop【";
      return test.eq((ƒ.seq(G.$step, G.$indent)).run(source), ['loop', '【']);
    },
