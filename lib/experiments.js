...
(function() {
  var A, TRM, alert, badge, debug, echo, element, error, g, glob, help, idx, info, log, name, new_tsort_graph, node, offender, rainbow, rpr, warn, whisper, π, _i, _len,
    __slice = [].slice;
  ...
  for (idx = _i = 0, _len = offender.length; _i < _len; idx = ++_i) {
    element = offender[idx];
    if (element === 'third') {
      offender[idx] += '%';
    }
    g['nodes']['third%'] = [];
  ...
  try {
    ...
    error = _error;
    if (!/^There is a cycle in the graph/.test(error['message'])) {
      throw error;
    }
    warn(g);
  }

  for (name in π.alt(function() {})) {
    debug(name);
  }
