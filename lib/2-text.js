...
(function() {
  var BNP, NEW, TRM, alert, badge, debug, echo, help, info, log, rainbow, rpr, warn, whisper, π;
  ...
  this._single_quote = π.alt((function(_this) {
    return function() {
  ...
  this._double_quote = π.alt((function(_this) {
    return function() {
  ...
  this._nosq = (π.alt(((function(_this) {
    ...
    return function() {
      return RegExp("[^" + (BNP.escape_regex(_this._constants['single-quote'])) + "]+");
    };
  ...
  this._nodq = (π.alt(((function(_this) {
    ...
    return function() {
      return RegExp("[^" + (BNP.escape_regex(_this._constants['double-quote'])) + "]+");
    };
  })(this)))).onMatch(function(match) {
    return match[0];
  });
  ...
  this._escaped = (π.seq(this._chr_escaper, π.alt(this._simple_escape, this._unicode_hex, /./))).onMatch((function(_this) {
    return function(match) {
      return match[1];
    };
  })(this));
  ...
  this.TESTS = {
    ...
    '_simple_escape: accepts and translates meta-chracters': function(test) {
    ...
    '_escaped: accepts escaped chracters': function(test) {
      ...
      for (_i = 0, _len = probes_and_results.length; _i < _len; _i++) {
        _ref = probes_and_results[_i], probe = _ref[0], result = _ref[1];
        debug(rpr(this._escaped.run(probe)));
        _results.push(test.eq(this._escaped.run(probe), result));
      }
      return _results;
    },
    '_nosq: accepts runs of chracters except unescaped single quote': function(test) {
      var probe, probes_and_results, result, _i, _len, _ref, _results;
      probes_and_results = [['0', '0'], ['qwertz', 'qwertz'], ["qw+nertz", "qw\nertz"], ['中華人"民共和國"', '中華人"民共和國"']];
      _results = [];
      for (_i = 0, _len = probes_and_results.length; _i < _len; _i++) {
        _ref = probes_and_results[_i], probe = _ref[0], result = _ref[1];
    ...
    '_nodq: accepts runs of chracters except unescaped double quote': function(test) {
      var probe, probes_and_results, result, _i, _len, _ref, _results;
