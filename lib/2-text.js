...
(function() {
  var $new, BNP, CHR, G, TRM, XRE, alert, badge, debug, echo, help, info, log, rainbow, rpr, warn, whisper, ƒ;
  ...
  this.literal = this.literal.onMatch((function(_this) {
    return function(match, state) {
      ...
      return _this.nodes.literal(state, value);
    };
  })(this));
  ...
  this.nodes.literal = function(state, value) {
  ...
  this.$TESTS = {
    ...
    '$escaped: accepts escaped chracters': function(test) {
      ...
      for (_i = 0, _len = probes_and_matchers.length; _i < _len; _i++) {
        ...
        result = ƒ["new"]._delete_grammar_references(this.$_escaped.run(probe));
        _results.push(test.eq(this.$_escaped.run(probe), matcher));
      }
      return _results;
    },
    ...
    '$literal: accepts single and double quoted string literals': function(test) {
      ...
      for (_i = 0, _len = probes_and_matchers.length; _i < _len; _i++) {
        _ref = probes_and_matchers[_i], probe = _ref[0], matcher = _ref[1];
        result = ƒ["new"]._delete_grammar_references(this.literal.run(probe));
        _results.push(test.eq(result, matcher));
      }
      return _results;
    },
    'as.coffee: render text literal as CoffeeScript': function(test) {
